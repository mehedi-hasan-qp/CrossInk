...
if args.bangla:
    ...
    # 3-consonant clusters: C1 + VIRAMA + C2 + VIRAMA + C3 → 5-codepoint sequences.
    # Key format: (1 << 63) | (cp1 << 42) | (cp3 << 21) | cp5  (cp2=cp4=VIRAMA, implicit).
    # Bit 63 as a 5-cp flag: all 3-cp keys are < 2^54 so bit 63 is always 0 there.
    _3C_FLAG = (1 << 63)
    _3c_entries = 0
    for _c1 in _CONSONANTS:
        for _c2 in _CONSONANTS:
            for _c3 in _CONSONANTS:
                _shaped5, _pos5 = _hb_shape_full([_c1, _VIRAMA, _c2, _VIRAMA, _c3])
                if len(_shaped5) >= 3:
                    continue  # not a real conjunct — skip
                if len(_shaped5) == 1:
                    _ft_idx = _shaped5[0]
                    if _ft_idx == _single_glyph.get(_c1, -1):
                    ...
                    if _ft_idx not in _ft_glyph_to_pua:
                        ...
                        _pua_cp = _ft_glyph_to_pua[_ft_idx]
                    bangla_cluster_entries.append((_c1, _VIRAMA, _c2, _VIRAMA, _c3, _pua_cp))
                    _3c_entries += 1
                elif len(_shaped5) == 2:
                    _comp_key = tuple(_shaped5)
                    if _comp_key in _composite_key_to_pua:
                        _pua_cp = _composite_key_to_pua[_comp_key]
                    else:
                        _pua_cp = _pua_next
                        g, packed = _render_composite_glyphs(ft_primary, _shaped5, _pos5, _pua_cp)
                        if g is None:
                            continue
                        _composite_key_to_pua[_comp_key] = _pua_cp
                        _pua_next += 1
                    ...
                    _3c_entries += 1
    if _3c_entries:
        print(f"bangla: {_3c_entries} 3-consonant cluster entries added", file=sys.stderr)
    ...
    if args.bangla_clusters_out and bangla_cluster_entries:
        with open(args.bangla_clusters_out, "w") as _cf:
            ...
            _cf.write("// Key encoding:\n")
            _cf.write("//   3-cp: key = (cp1 << 42 | cp2 << 21 | cp3),           cp2 = VIRAMA\n")
            _cf.write("//   5-cp: key = (1ULL<<63) | (cp1<<42) | (cp3<<21) | cp5, cp2=cp4=VIRAMA (implicit)\n")
            ...
            def _entry_sort_key(e):
                if len(e) == 4:
                    return (e[0] << 42) | (e[1] << 21) | e[2]
                else:  # 6-tuple: 5-cp cluster
                    return _3C_FLAG | (e[0] << 42) | (e[2] << 21) | e[4]
            for entry in sorted(bangla_cluster_entries, key=_entry_sort_key):
                if len(entry) == 4:
                    _c1, _via, _c2, _pua = entry
                    _key = (_c1 << 42) | (_via << 21) | _c2
                    _cf.write(f"    {{ 0x{_key:016X}ULL, 0x{_pua:04X} }},  "
                              f"// {chr(_c1)}্{chr(_c2)} → U+{_pua:04X}\n")
                else:
                    _c1, _via, _c2, _via2, _c3, _pua = entry
                    _key = _3C_FLAG | (_c1 << 42) | (_c2 << 21) | _c3
                    _cf.write(f"    {{ 0x{_key:016X}ULL, 0x{_pua:04X} }},  "
                              f"// {chr(_c1)}্{chr(_c2)}্{chr(_c3)} → U+{_pua:04X}\n")
            _cf.write("};\n\n")
